...
function isInsideCircle(circle, point) {
  return (((point.x - circle.center.x)
...
function isCreditCardNumber(ccn) {
  return ((ccn.toString().split('').reverse().reduce((acc, item, i) => {
    if (i % 2) {
      const val = parseInt(item, 10) * 2;
      if (val > 9) return acc + val - 9;
      return acc + val;
    }
...
function getDigitalRoot(num) {
  let result = num;
  if (num >= 10) {
    result = getDigitalRoot(parseInt(num.toString().split('')
      .reduce((acc, item) => acc + parseInt(item, 10), 0), 10));
  }
  return result;
}
...
function toNaryString(num, n) {
  ...
  let item = num;
  while (item > 0) {
    result += (item % n.toString());
    item = Math.floor(item / n);
  }
...
function getCommonDirectoryPath(pathes) {
  let result = '';
  pathes[0].split('').some((item, index) => {
    if (pathes.every((arr) => arr[index] === item)) {
      result = `${result}${item}`;
      return false;
    }
    return true;
